...
// 使用SETNX命令设置锁，如果成功返回1，否则返回0
func (r *Redis) SetNX(key string, value interface{}, lockDuration time.Duration) (bool, error) {
	return r.Client.SetNX(ctx, key, value, lockDuration).Result()
}

// 获取范围内所有已使用的金额
func (r *Redis) ZRangeByScore(key string, min, max string) ([]string, error) {
